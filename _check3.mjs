...
const USA_IDS = [
  'agent_9601k2ehgvjwe7tvft51fvycv9ve',
  'agent_2101k2egr1ryfd188w5x88pegza1',
];
...
for (const id of USA_IDS) {
  ...
  if (!agent?.agent_id) { console.log(`NOT FOUND: ${id}\n`); continue; }
  const name = agent.name;
  const kbCount = agent.conversation_config?.agent?.prompt?.knowledge_base?.length ?? 0;

  if (euByName.has(name.toLowerCase().trim())) {
    const eu = euByName.get(name.toLowerCase().trim());
    ...
    console.log(`  Diffs: ${diffs.length ? diffs.join(', ') : 'IDENTICAL'}`);
  } else {
